...
# Svript name
  SCRIPTNAME="OwnCloud Backup Script"
# Version
  VERSION=0.01
...
if [ $DBBACKUP -eq 1 ]; then
    OUTFILE=$TODAYSBACKUPDIR/OwnCloudDB.`date +"%Y%m%d_%H%M%S"`.bak
    echo " "
    echo " "
    echo "#######################################################################"
    echo "$STARTTIME: Creating a backup of the OwnCloud Database to $OUTFILE" 
    echo "#######################################################################"
